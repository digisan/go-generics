...
func DelEleOrderlyAt(arr *[]string, i int) {
	*arr = append((*arr)[:i], (*arr)[i+1:]...)
}

func DelEleAt(arr *[]string, i int) {
	(*arr)[i] = (*arr)[len(*arr)-1]
	(*reflect.SliceHeader)(unsafe.Pointer(arr)).Len--
}

func DelOneEle(arr *[]string, ele string) {
	if idx := IdxOf(ele, (*arr)...); idx >= 0 {
		DelEleAt(arr, idx)
	}
}

func DelOneEleOrderly(arr *[]string, ele string) {
	if idx := IdxOf(ele, (*arr)...); idx >= 0 {
...
func Filter(data *[]string, check func(i int, e string) bool) []string {
	if check == nil {
		return *data
	}
...
// MapFilter:
func MapFilter(m map[string]string, filter func(k string, v string) bool) map[string]string {
...
func MapCopy(m map[string]string) map[string]string {
	return MapFilter(m, func(k string, v string) bool { return true })
}

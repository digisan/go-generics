...
// FilterMap : Filter & Modify []byte slice, return []int slice
func FilterMap(arr []byte, filter func(i int, e byte) bool, modifier func(i int, e byte) int) (r []int) {
	if modifier == nil {
...
func Map2KVs(m map[byte]int, less4key func(i byte, j byte) bool, less4value func(i int, j int) bool) (keys []byte, values []int) {
...
// MapMerge:
func MapMerge(ms ...map[byte]int) map[byte][]int {
	res := map[byte][]int{}
	for _, m := range ms {
	srcMap:
		for k, v := range m {
...
// MapFilter:
func MapFilter(m map[byte]int, filter func(k byte, v int) bool) map[byte]int {
	rt := make(map[byte]int)
	for k, v := range m {
		if filter(k, v) {
			rt[k] = v
		}
...
// MapCopy:
func MapCopy(m map[byte]int) map[byte]int {
	return MapFilter(m, func(k byte, v int) bool { return true })
}
